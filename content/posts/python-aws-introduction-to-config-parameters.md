...
In this article I want to analyze some concepts that you can use for your implementation. The implementation used here will show how to apply those concepts when using **Python**, **AWS** and **boto3** as your tech stack.
...
- If multiple programs use the same value, you need to edit each program's source code when changing your parameter.
...
We can use any file format we want. [JSON](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON), [YAML](https://yaml.org/spec/1.2.2/) and [TOML](https://toml.io/en/) are some common formats you can use to define your configuration file.
...
Advantages:
- Value can change without editing code.
...
You can find more information about boto3 and s3 [here](https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/s3.html#s3).
...
Environment variables can easily be defined and kept secret to the outside world (unless someone gains access to the machine and you might have even bigger problems). You can define them on your **local machine** when testing your software, in **Dockerfile** and **docker-compose** for easy deployment, in your **CI/CD** pipeline or in cloud providers container solutions (see [here](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/taskdef-envfiles.html) for an example with **AWS ECS**).
...
2. We can inject it into the *config creator* program, that will then push the configuration file with the variable to S3.
...
You can refer to [AWS documentation](https://docs.aws.amazon.com/systems-manager/latest/userguide/systems-manager-parameter-store.html) for how to save parameters there. Let's see how we can then use them in our Python application.
...
# Caching: SSM Cache
We can solve this last issue by using a [cache](https://en.wikipedia.org/wiki/Cache_(computing)).
